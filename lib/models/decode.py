...
def _topk(scores, K=40):
    # 这个函数的作用就是先分别找每个类别heatmap值的前100，然后在这些前100中，找所有类的前100heatmap值
    #import ipdb
    #ipdb.set_trace()
    batch, cat, height, width = scores.size()

    #先在不同类别中找前K, topk_inds: (batch, cat, K)
    topk_scores, topk_inds = torch.topk(scores.view(batch, cat, -1), K)
...
def corner_decode(mk, st_reg, mk_reg=None, K=400):
    ...
    # st_reg应该是corner角点的bbox四个位置坐标相对corner坐标的距离, 本质上与本模型中对目标中心预测的长宽的8个数值的wh做的是同一件事
    # st_reg:(batch, 8, H, W) -> st_Reg: (batch, K, 8)
...
def ctdet_4ps_decode(heat, wh, ax, cr, corner_dict=None, reg=None, cat_spec_wh=False, K=100, wiz_rev = False, use_gt_det = False):
            ...
            if scores[0,i,0] >= 0.2:
                ...
                for j in range(num_gboxes):
                    if corner_scores[0,j,0] >= 0.3:
                        ...
                        #  is_group_faster_faster判断角点的gbox与目标框bbox是否有交集，是一个加速匹配过程的一个条件检验函数
                        if is_group_faster_faster(bbox, gbox): 
                            #step2 find which corner point to refine, and do refine
                            #校准bboxes，认为直接预测的wh还有误差，而在前面专门把corner角点作为第二类目标检测，拿预测的角点坐标来匹配bboxes里最接近的角点，然后校准它
                            ...
                            if bboxes_rev[0, i, 2*ind4ps] == bboxes[0, i, 2*ind4ps] and bboxes_rev[0, i, 2*ind4ps+1] == bboxes[0, i, 2*ind4ps+1]:
                                #first_shift
                                # 因为可能有多个gbox与bbox有交集，一个角点可能被校准多次，所以会有是否第一次校准的判断
                                count = count + 1
    ...
    if wiz_rev and not use_gt_det:
        # 采用Cycle-CenterNet的模型，针对有线表，对角点的精度要求就比较高，所以就用到了前面的校准过程
    ...
    # cc_match就是center-corner（1比4）match，即每个目标中心点对应的四个角点的坐标的flatten索引表示
    cc_match = torch.round(cc_match).to(torch.int64)

    # cc_match: (batch, K, 4), cr: (batch, 256, H, W), 此处K为center目标的数量
    cr_feat = _get_4ps_feat(cc_match, cr)
    # cr_feat: (batch, K, 256, 4)->(batch, K, 256)，表示每个单元格目标对应的四个角点的feat求sum
    cr_feat = cr_feat.sum(axis = 3)
    if wiz_rev and not use_gt_det:
        # 在前面wiz_rev的条件下，bboxes_rev校准过程中，scores是发生变化的，所以需要重新排降序
