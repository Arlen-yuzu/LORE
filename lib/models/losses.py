...
from .utils import _flatten_and_gather_feat, _tranpose_and_gather_feat, _make_pair_feat
...
class AxisLoss(nn.Module):
  ...
  def forward(self, output, mask, ind, target, logi=None):
    ...
    #construct span data
    target_width = target[:,:,1] - target[:,:,0] # batch x max_objects
    target_height = target[:,:,3] - target[:,:,2]
    width = pred[:,:,1] - pred[:,:,0]
    height = pred[:,:,3] - pred[:,:,2]
    target_span = torch.cat((target_width.unsqueeze(2), target_height.unsqueeze(2)), axis = 2) # batch x max_objects x 2
    ...
    if span_type :
      ...
      major_loss = major_loss/ (2*(major_mask.sum() + 1e-4))

      minor_loss = F.l1_loss(span * minor_mask, target_span * minor_mask, size_average=False)
      minor_loss = minor_loss/ (2*(minor_mask.sum() + 1e-4))
      loss_span = major_loss + minor_loss
      # loss_span = minor_loss
...
class RegL1Loss(nn.Module):
  ...
  def forward(self, output, mask, ind, target):
    # output (batch x dim x h x w) -> pred (batch x max_objects x dim)
    pred = _tranpose_and_gather_feat(output, ind)
...
class DistLoss(nn.Module):
  ...
  def rank_dist(self, pred, pair, pair_ind, rtype = 'horizontal'):
    # pred: batch x max_objects x 8
    # pair: batch x max_objects x 8, ground truth
    # pair_ind: batch x max_objects
    # rtype: 'horizontal' or 'vertical'

    if rtype == 'horizontal':
      pred_1 = pred[:, :, 6] - pred[:, :, 3] # cs2 - ce1
      pred_2 = pred[:, :, 2] - pred[:, :, 7] # cs1 - ce2
      gt_right_judge = pair[:, :, 3] < pair[:, :, 6]
      dist =  torch.where(gt_right_judge, pred_1, pred_2)
    elif rtype == 'vertical':
      pred_1 = pred[:, :, 4] - pred[:, :, 1] # rs2 - re1
      pred_2 = pred[:, :, 0] - pred[:, :, 5] # rs1 - re2
      gt_down_judge = pair[:, :, 1] < pair[:, :, 4]
      dist =  torch.where(gt_down_judge, pred_1, pred_2)
    else:
      raise ValueError('rtype should be horizontal or vertical')
  ...
  def forward(self, h_pair_ind, v_pair_ind, logic, logi=None):
    ...
    h_pred = _flatten_and_gather_feat(pred_pair, h_pair_ind) # batch x max_objects x 8
    ...
    h_dist = self.rank_dist(h_pred, h_pair, h_pair_ind, rtype = 'horizontal').squeeze() # batch x max_objects
    v_dist = self.rank_dist(v_pred, v_pair, v_pair_ind, rtype = 'vertical').squeeze()
