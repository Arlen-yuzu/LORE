...
def _get_4ps_feat(cc_match, output):
  ...
  feat = feat.unsqueeze(3).expand(feat.size(0), feat.size(1), feat.size(2), 4) # (batch, HxW, 256, 4)

  dim = feat.size(2)
  cc_match = cc_match.unsqueeze(2).expand(cc_match.size(0), cc_match.size(1), dim, cc_match.size(2)) # (batch, K, 256, 4)
...
def _normalized_ps(ps, vocab_size):
  ...
  ps = torch.where(ps < vocab_size, ps, (vocab_size-1) * torch.ones(ps.shape).to(torch.int64).to(ps.device))
  ps = torch.where(ps >= 0, ps, torch.zeros(ps.shape).to(torch.int64).to(ps.device))
  return ps
