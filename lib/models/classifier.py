    ...
    def forward(self, outputs, batch = None, cc_match = None, dets = None): #training version forward
      ...
      if batch is None:
        ...
        if dets is None:
          ...
          right_pe = self.x_position_embeddings(dets[:, :, 4])
        ...
        #Training Mode
        # bbox坐标用的gt，vis_feat用的是模型输出的
        ind = batch['hm_ind']
        mask = batch['hm_mask'] #during training, the attention mask will be applied
        output = outputs[-1]
        pred = output['ax']
        ct_feat = _tranpose_and_gather_feat(pred, ind) # (bs, dim, H, W) -> (bs, num_obj, dim)
        ...
        if self.opt.wiz_2dpe:        
          ...
          if self.opt.shift_bbox:
            random_offset = (torch.rand(ps.shape[0], ps.shape[1], 4) * 0.2 - 0.1 + 1.0).to(ps.device)
            bbox_gt_random_offset = random_offset[:, :, [0, 1, 2, 1, 2, 3, 0, 3]]
            ps = ps * bbox_gt_random_offset
          ...
          right_pe = self.x_position_embeddings(ps[:, :, 4])
