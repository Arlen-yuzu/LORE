...
class BaseTrainer(object):
  ...
  def run_epoch(self, phase, epoch, data_loader):
    ...
    if phase == 'train':
      ...
      if self.opt.only_processor:
        if isinstance(model_with_loss, torch.nn.parallel.distributed.DistributedDataParallel) \
           or isinstance(model_with_loss, torch.nn.parallel.DataParallel):
          model_with_loss.module.model.eval()
          model_with_loss.module.processor.train()
        else:
          model_with_loss.model.eval()
          model_with_loss.processor.train()
      else:
        model_with_loss.train()
    else:
