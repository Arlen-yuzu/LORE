...
from models.decode import ctdet_decode, ctdet_4ps_decode
...
class CtdetLoss(torch.nn.Module):
  def __init__(self, opt):
    ...
    self.crit_dist = DistLoss() # inter-loss
  ...
  def forward(self, epoch, outputs, batch, logi=None, slogi=None):
    ...
    for s in range(opt.num_stacks):
      ...
      if self.opt.wiz_pairloss:
        ...
        # only supervision on centers
        hm_loss += self.crit(output['hm'][:,0:self.opt.num_classes-1,:,:], batch['hm'][:,0:self.opt.num_classes-1,:,:]) / opt.num_stacks
      ...
      sm_loss = self.crit_dist(batch['h_pair_ind'], batch['v_pair_ind'], batch['logic'], logi)
     
      '''COMBINING LOSSES'''
      if opt.only_detector:
        loss = opt.hm_weight * hm_loss + opt.wh_weight * wh_loss + opt.off_weight * off_loss
        loss_stats = {'loss': loss, 'hm_l': hm_loss,  'wh_l': wh_loss}
        if self.opt.wiz_pairloss:
          loss = loss + st_loss
          loss_stats['st_l'] = st_loss
      
      elif opt.only_processor:
        if not self.opt.wiz_stacking:
          loss = 2 * ax_loss + sp_loss + sm_loss
          loss_stats = {'loss': loss, 'ax_l': ax_loss, 'sp_l': sp_loss, 'sm_l': sm_loss}
        else:
          loss = ax_loss + 2 * sax_loss + ss_sp_loss# + sm_loss
          loss_stats = {'loss': loss, 'ax_l': ax_loss, 'sax_l': sax_loss, 'sp_l': ss_sp_loss, 'sm_l': sm_loss}
        ...
        if self.opt.wiz_stacking:
          ...
          # sax_loss, ss_sp_loss = self.crit_ax(output['ax'], batch['hm_mask'], batch['hm_ind'], batch['logic'], slogi)
          # sm_loss = self.crit_dist(batch['h_pair_ind'], batch['v_pair_ind'], batch['logic'], logi)
          loss = loss + 3 * sax_loss + ss_sp_loss + sm_loss
          loss_stats['sax_l'] = sax_loss
          loss_stats['sp_l'] = ss_sp_loss
          loss_stats['sm_l'] = sm_loss
          # sacc = _axis_eval(output['ax'], batch['hm_mask'], batch['hm_ind'], batch['logic'], slogi)
        else:
          loss = loss + 2 * sp_loss + 2 * sm_loss
          loss_stats['sp_l'] = sp_loss
          loss_stats['sm_l'] = sm_loss
    ...
    # if self.opt.wiz_pairloss :
    #   loss_stats = {'loss': loss, 'hm_l': hm_loss,  'wh_l': wh_loss, "st_l": st_loss, "ax_l": ax_loss, "sp_l": ss_sp_loss, "sm_l": sm_loss}
    # else:
    #   loss_stats = {'loss': loss, 'hm_l': hm_loss,  'wh_l': wh_loss, "ax_l": ax_loss, "sp_l": ss_sp_loss, "sm_l": sm_loss} 
...
class CtdetTrainer(BaseTrainer):
  ...
  def _get_losses(self, opt):
    ...
    if opt.only_detector:
      if opt.wiz_pairloss:
        loss_stats = ['loss', 'hm_l', 'wh_l',  'st_l']
      else:
        loss_stats = ['loss', 'hm_l', 'wh_l']
    
    elif opt.only_processor:
      if opt.wiz_stacking:
        loss_stats = ['loss', 'ax_l', 'sp_l', 'sm_l', 'sax_l']
      else:
        loss_stats = ['loss', 'ax_l', 'sp_l', 'sm_l']
    
    else:
      if opt.wiz_pairloss:
        loss_stats = ['loss', 'hm_l', 'wh_l',  'st_l', 'ax_l', 'sp_l', 'sm_l']
      else:
        loss_stats = ['loss', 'hm_l', 'wh_l',  'ax_l', 'sp_l', 'sm_l']
  ...
  def save_result(self, output, batch, results):
    ...
    # change from ctdet_decode to ctdet_4ps_decode
    dets, _, _, _ = ctdet_4ps_decode(
      output['hm'], output['wh'], reg=reg,
