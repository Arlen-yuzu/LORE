...
class CTDetDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    for k in range(num_objs):
      ...
      for kk in range(k+1, num_objs):
        rs1, re1, cs1, ce1 = ann['logic_axis'][0][0], ann['logic_axis'][0][1], ann['logic_axis'][0][2], ann['logic_axis'][0][3]
        ann_ = anns[kk]
        rs2, re2, cs2, ce2 = ann_['logic_axis'][0][0], ann_['logic_axis'][0][1], ann_['logic_axis'][0][2], ann_['logic_axis'][0][3]

        # 判断上下相邻
        if cs1 <= cs2 <= ce1 or cs2 <= cs1 <= ce2:
          if (rs1 - re2 == 1) or (rs2 - re1 == 1):
            v_pair_ind[v_ind] = k*num_objs + kk
            v_ind += 1
          
        # 判断左右相邻
        if rs1 <= rs2 <= re1 or rs2 <= rs1 <= re2:
          if (cs1 - ce2 == 1) or (cs2 - ce1 == 1):
            h_pair_ind[h_ind] = k*num_objs + kk
            h_ind += 1
