...
class Table(data.Dataset):
  num_classes = 3 # change class num
  table_size = 1024
  ...
  def __init__(self, opt, split):
    ...
    # self.img_dir = os.path.join(opt.image_dir, _ann_name[split])
    self.img_dir = opt.image_dir
    print(self.img_dir)
    # self.annot_path = os.path.join(self.data_dir, 'json', '{}.json').format(_ann_name[split])
    self.annot_path = os.path.join(opt.anno_path, '{}.json'.format(_ann_name[split]))
    print(self.annot_path)
    
    self.max_objs = 500
    self.max_pairs = 1500
    self.max_cors = 2000 #800 #1200 #800
    
    # if self.num_classes == 3:
    #   # change classes， 这个class_name目前貌似没有使用
    #   self.class_name = [
    #     '__background__', 'center', 'header', 'corner']
    #   self._valid_ids = [0,1,2] # 原来是1,2, 用于转换回原来类别标注，原来是从1开始标注cls的，但是目前新生成的数据从0开始标
    # else:
    #   self.class_name = [
    #     '__background__', 'center', 'corner']
    #   self._valid_ids = [0,1]
      
    self._valid_ids = [i for i in range(self.num_classes)]
  ...
  def convert_eval_format(self, all_bboxes, thresh):
    ...
    for image_id in all_bboxes:
      for cls_ind in all_bboxes[image_id]:
        category_id = self._valid_ids[cls_ind] # change from cls_ind-1 to cls_ind
